...
/**
 * This class is used when the list of pipes for a given host is requested.<p>
 * It contains a class of type {@link ItemInfo},<p>
 * a List of {@link ItemInfo} containing data for all the pipes of an host,<p>
 * and the number of pipes in that List.
 */
...
public class PipesInfo {
    ...
    /**
     * This class contains information for a single pipe:<p>
     * The identity of the pipe<p>
     * The State of the pipe (opened, opened and created, opened/created and RF active)<p>
     * The identity of the source host<p>
     * The identity of the source gata<p>
     * The identity of the destination host.
     */
    ...
    public PipesInfo(int nb) {
        if (DBG)
            Log.i(tag, "Contructor - nbPipes = " + nb);
        nbPipes = nb;
    }
     ...
     *
     * @param idx This parameter identifies an entry in the info field of the class
     * {@link PipesInfo}.
     * @return A String containing all the information about the gate located at index {@link idx}.
     */
    public String getPipeData(int idx) {
        String data = "    Gate: 0x" + info.get(idx).sourceGate + "\n      pipe Id: 0x" + info.get(
                idx).pipeId
                + " - " + info.get(idx).pipeState +
                "\n      Source Host: " + info.get(idx).sourceHost + " - Dest Host: " + info.get(
                idx).destHost;
    ...
    /**
     * This method is used to retrieve information from the field info of the class
     * {@link PipesInfo}
     * for the pipe stored at index {@link pipe_idx}.
     *
     * @param idx This parameter identifies an entry in the info field of the class
     * {@link PipesInfo}.
    ...
    public ItemInfo getPipeInfo(int pipe_idx) {
        if (DBG)
            Log.i(tag, "getPipeInfo()");
        return info.get(pipe_idx);
    }

    /**
     * This method stores information contained in the {@link data} parameter inside
     * the info field of the {@link PipesInfo} class for the pipe identified by the
     * {@link pipe_id} parameter.
     *
     * @param pipe_id This parameter identifies the pipe.
     * @param data    This parameter is an array of bytes containing information (pipe state,
     *                source gate, host)
     *                about the pipe identified by the parameter {@link pipe_id}.
     */
