...
public final class StApduServiceInfo implements Parcelable {
    ...
    public StApduServiceInfo(
        ...
        if(DBG) Log.d(TAG, "Constructor - offHost: " + offHost + ", description: " + description +
            ", nb of staticStAidGroups: " + staticStAidGroups.size() +
            ", nb of dynamicStAidGroups: " + dynamicStAidGroups.size());
    ...
    public boolean hasCategory(String category) {
        return (mStaticStAidGroups.containsKey(category) || mDynamicStAidGroups.containsKey(
                category));
    }
    ...
    public ApduServiceInfo createApduServiceInfo() {
        return new ApduServiceInfo(
                ...
                this.getSettingsActivityName(), this.getOffHostSecureElement(),
     ...
     *
     * @param category The category of the corresponding service
     *                 .{@link #CardEmulation.CATEGORY_OTHER}.
     * @return The aid cache size for particular category.
     */
    public int getAidCacheSize(String category) {
        int aidSize = 0x00;
        if (!CardEmulation.CATEGORY_OTHER.equals(category) || !hasCategory(
    ...
    /**
     * This api gives the size occupied by this service's AIDs in routing table
     * Only the AIDs of  {@link #CardEmulation.CATEGORY_OTHER} are accounted.
     * This API does not show the really used size, just what AID size is if it needs to be stored.
     *
    ...
    public int getCatOthersAidSizeInLmrt() {
        ...
        return getAidCacheSizeForCategory(CardEmulation.CATEGORY_OTHER) + getTotalAidNum(
                CardEmulation.CATEGORY_OTHER) * 4;
    }
    ...
    public boolean isServiceEnabled(String category) {
        ...
        if ((mServiceState == StConstants.SERVICE_STATE_ENABLED) || (mServiceState
                == StConstants.SERVICE_STATE_DISABLING)) {
            return true;
    ...
    public void enableService(String category, boolean flagEnable) {
        ...
        Log.d(TAG, "setServiceState:Description:" + mDescription + ":InternalState:" + mServiceState
        ...
        if (((mServiceState == StConstants.SERVICE_STATE_ENABLED) && (flagEnable == true)) ||
                ...
                ((mServiceState == StConstants.SERVICE_STATE_ENABLING) && (flagEnable == true))) {
            /*No change in state*/
            return;
     ...
     * This method needs to be invoked after current service is pushed for the commit to routing
     * table
     *
    ...
    public void updateServiceCommitStatus(String category, boolean commitStatus) {
        ...
        Log.d(TAG, "updateServiceCommitStatus(enter) - Description: " + mDescription + ", InternalState: "
                + mServiceState + ", commitStatus: " + commitStatus);
